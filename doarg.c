...
int
do_subst(STR *str, ARG *arg, int sp)
{
    register SPAT *spat;
    SPAT *rspat;
    register STR *dstr;
	...
	if (hint < s || hint > strend)
	    fatalx(0, "panic: hint in do_match");
	s = hint;
	hint = Nullch;
...
void
do_join(register STR *str, int *arglast)
{
...
void
do_pack(register STR *str, int *arglast)
{
    register STR **st = stack->ary_array;
...
#define NEXTFROM (items-- > 0 ? *st++ : &str_no)
	...
	default:
	    break;
	case '%':
	    fatalx(0, "% may only be used in unpack");
	case '@':
	    len -= str->str_cur;
	    if (len > 0)
		goto grow;
...
static void
doencodes(register STR *str, register char *s, register int len)
{
    char hunk[5];
...
int *arglast;
{
    ...
    if (!stab)
	fatalx(0, "Undefined subroutine called");
    if (!(sub = stab_sub(stab))) {
	STR *tmpstr = arg[0].arg_ptr.arg_str;

	stab_efullname(tmpstr, stab);
	fatalx(1, "Undefined subroutine \"%s\" called",tmpstr->str_ptr);
    }
	...
	if (sub->depth >= 2) {	/* save temporaries on recursion? */
		...
		warnx(1, "Deep recursion on subroutine \"%s\"",stab_ename(stab));
...
int
do_assign(register ARG *arg, int gimme, int *arglast)
{
...
#define NSIG 32
	...
	if (delaymagic & DM_UID) {
	    ...
	    if (delaymagic & DM_UID) {
		if (uid != euid)
		    fatalx(0, "No setreuid available");
		(void)setuid(uid);
	    }
	...
	if (delaymagic & DM_GID) {
	    ...
	    if (delaymagic & DM_GID) {
		if (gid != egid)
		    fatalx(0, "No setregid available");
		(void)setgid(gid);
	    }
...
int					/*SUPPRESS 590*/
do_study(STR *str, ARG *arg, int gimme, int *arglast)
{
    ...
    if (!sfirst || !snext)
	fatalx(0, "do_study: out of memory");
...
int					/*SUPPRESS 590*/
do_defined(STR *str, register ARG *arg, int gimme, int *arglast)
{
    register int type;
	...
	fatalx(0, "Illegal argument to defined()");
...
int						/*SUPPRESS 590*/
do_undef(STR *str, register ARG *arg, int gimme, int *arglast)
{
    register int type;
    register STAB *stab;
    int retarg = arglast[0] + 1;
    ...
    if ((arg[1].arg_type & A_MASK) != A_LEXPR)
	fatalx(0, "Illegal argument to undef()");
    arg = arg[1].arg_ptr.arg_arg;
    type = arg->arg_type;
	...
	fatalx(0, "Can't undefine that kind of object");
    str_numset(str,0.0);
    stack->ary_array[retarg] = str;
    return retarg;
}
...
int
do_vec(int lvalue, STR *astr, int *arglast)
{
...
void
do_chop(register STR *astr, register STR *str)
{
    register char *tmps;
    register int i;
    ARRAY *ary;
...
void
do_vop(int optype, STR *str, STR *left, STR *right)
{
    register char *s;
...
do_syscall(int *arglast)
{
    ...
    switch (items) {
    case 0:
	fatalx(0, "Too few args to syscall");
    case 1:
    ...
    return retval;
#else
    fatalx(0, "syscall() unimplemented");
#endif
}
