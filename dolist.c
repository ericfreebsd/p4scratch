...
int
do_match(STR *str, register ARG *arg, int gimme, int *arglast)
{
...
int
do_split(STR *str, register SPAT *spat, register int limit, int gimme, int *arglast)
{
...
int
do_unpack(STR *str, int gimme, int *arglast)
{
...
int
do_slice(STAB *stab, STR *str, int numarray, int lval, int gimme, int *arglast)
{
    register STR **st = stack->ary_array;
...
int
do_splice(register ARRAY *ary, int gimme, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    int max = arglast[2] + 1;
...
int
do_grep(register ARG *arg, STR *str, int gimme, int *arglast)
{
    STR **st = stack->ary_array;
...
int
do_sreverse(STR *str, int *arglast)
{
    STR **st = stack->ary_array;
    register char *up;
    register char *down;
...
int
do_sort(STR *str, ARG *arg, int gimme, int *arglast)
{
    register STR **st = stack->ary_array;
    int sp = arglast[1];
    register STR **up;
	...
	if (arg[1].arg_type == (A_CMD|A_DONT)) {
		...
		if (!stab_sub(stab) || !(sortcmd = stab_sub(stab)->cmd))
		    fatalx(1, "Undefined subroutine \"%s\" in sort",
			stab_ename(stab));
...
int
do_range(int gimme, int *arglast)
{
    STR **st = stack->ary_array;
...
int
do_caller(ARG *arg, int maxarg, int gimme, int *arglast)
{
    STR **st = stack->ary_array;
    register int sp = arglast[0];
...
int
do_tms(STR *str, int gimme, int *arglast)
{
#ifdef MSDOS
    return -1;
#else
...
int
do_time(STR *str, struct tm *tmbuf, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
    STR **st = ary->ary_array;
    register int sp = arglast[0];
...
int
do_kv(STR *str, HASH *hash, int kv, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
    STR **st = ary->ary_array;
    register int sp = arglast[0];
...
int
do_each(STR *str, HASH *hash, int gimme, int *arglast)
{
    STR **st = stack->ary_array;
    register int sp = arglast[0];
