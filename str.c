...
#ifndef DOSISH
char *
str_grow(register STR *str, register int newlen)
#else
char *
str_grow(register STR *str, unsigned long newlen)
#endif
{
...
str_2ptr(register STR *str)
{
	...
	if (str == &str_undef)
	    return No;
	if (dowarn)
	    warnx(0, "Use of uninitialized variable");
	STR_GROW(str, 30);
	s = str->str_ptr;
    }
    *s = '\0';
...
void
str_nset(register STR *str, register char *ptr, register STRLEN len)
{
    if (str == &str_undef)
	return;
    STR_GROW(str, len + 1);
...
void
str_chop(register STR *str, register char *ptr)
{
...
void
str_ncat(register STR *str, register char *ptr, register STRLEN len)
{
    if (str == &str_undef)
	return;
    if (!(str->str_pok))
...
char *
str_append_till(register STR *str, register char *from, register char *fromend, register int delim, char *keeplist)
{
    register char *to;
    register STRLEN len;
...
void
str_magic(register STR *str, STAB *stab, int how, char *name, STRLEN namlen)
{
...
int
str_eq(register STR *str1, register STR *str2)
{
...
char *
str_gets(register STR *str, register FILE *fp, int append)
{
...
STR *str;
{
	...
	fatalx(0, "Invalid component in string or format");
    cmd = eval_root;
    arg = cmd->c_expr;
    if (cmd->c_type != C_EXPR || cmd->c_next || arg->arg_type != O_LIST)
	fatalx(3, "panic: error in parselist %d %x %d", cmd->c_type,
...
STR *src;
{
	...
	if (*s == '\\' && s[1] && index("$@[{\\]}lLuUE",s[1])) {
		...
		do {
			...
			if (s >= send)
			    fatalx(0, "Unterminated string");
			break;
		    }
		    s++;
		} while (brackets > 0 && s < send);
		if (s > send)
		    fatalx(0, "Unmatched brackets in string");
...
int sp;
{
	...
	if (*s == '$' && s+1 < send) {
	    ...
	    switch(*++s) {
	    default:
		fatalx(0, "panic: unknown interp cookie\n");
		break;
	    case 'a':
		str_scat(str,*++elem);
		break;
...
static void
ucase(register char *s, register char *send)
{
    while (s < send) {
	if (isLOWER(*s))
	    *s = toupper(*s);
...
static void
lcase(register char *s, register char *send)
{
    while (s < send) {
	if (isUPPER(*s))
	    *s = tolower(*s);
...
str_smake(register STR *old)
{
	...
	warnx(0, "semi-panic: attempt to dup freed string");
	return Nullstr;
    }
...
void
str_reset(register char *s, HASH *stash)
{
    register HENT *entry;
    register STAB *stab;
    register STR *str;
