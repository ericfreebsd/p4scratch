...
make_sub(char *name, CMD *cmd)
{
	...
	if (dowarn) {
	    ...
	    if (cmd)
		curcmd = cmd;
	    warnx(1, "Subroutine %s redefined",name);
	    curcmd = oldcurcmd;
	}
...
SUBR *
make_usub(char *name, int ix, int (*subaddr)(), char *filename)
{
    register SUBR *sub;
 ...
 */
static void
make_cswitch(register CMD *head, int count)
{
    register CMD *cur;
    register CMD **loc;
    register int i;
...
static void
make_nswitch(register CMD *head, int count)
{
...
CMD *
append_line(register CMD *head, register CMD *tail)
{
    if (tail == Nullcmd)
	return head;
...
CMD *
make_acmd(int type, STAB *stab, ARG *cond, ARG *arg)
{
    register CMD *cmd;
...
CMD *
make_ccmd(int type, int debuggable, ARG *arg, struct compcmd cblock)
{
    register CMD *cmd;
...
CMD *
make_icmd(int type, ARG *arg, struct compcmd cblock)
{
    register CMD *cmd;
    register CMD *alt;
    register CMD *cur;
...
void
opt_arg(register CMD *cmd, int fliporflop, int acmd)
{
    register ARG *arg;
    int opt = CFT_EVAL;
    int sure = 0;
...
CMD *
add_label(char *lbl, register CMD *cmd)
{
    if (cmd)
	cmd->c_label = lbl;
    return cmd;
...
CMD *
addcond(register CMD *cmd, register ARG *arg)
{
    cmd->c_expr = arg;
    cmd->c_flags |= CF_COND;
    return cmd;
...
CMD *
addloop(register CMD *cmd, register ARG *arg)
{
    cmd->c_expr = arg;
...
void
cpy7bit(register char *d, register char *s, register int l)
{
    while (l--)
	*d++ = *s++ & 127;
    *d = '\0';
...
yyerror(char *s)
{
	...
	fputs(buf,stderr);
    if (++error_count >= 10)
	fatalx(1, "%s has too many errors.\n",
	stab_val(curcmd->c_filestab)->str_ptr);
}

void
while_io(register CMD *cmd)
{
...
CMD *
over(STAB *eachstab, register CMD *cmd)
{
...
static int
cmd_tosave(register CMD *cmd, int willsave)
{
    register CMD *head = cmd;
...
static int
arg_tosave(register ARG *arg, int willsave)
{
    register int i;
...
static int
spat_tosave(register SPAT *spat)
{
    int shouldsave = FALSE;
