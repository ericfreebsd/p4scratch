 ...
 */
regexp *
regcomp(char *exp, char *xend, int fold)
{
	register regexp *r;
	register char *scan;
	...
	if (op == '{' && regcurly(regparse)) {
		...
		if (flags&SIMPLE) {	/* we can do it right after all */
			...
			*flagp = (WORST|HASWIDTH);
		    ...
		    if (tmp && tmp < iter)
			fatalx(0, "Can't do {n,m} with n > m");
		    if (regcode != &regdummy) {
#ifdef REGALIGN
...
static void
regset(char *bits, int def, register int c)
{
	if (regcode == &regdummy)
	    return;
	c &= 255;
