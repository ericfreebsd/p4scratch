...
void
stabset(register STR *mstr, STR *str)
{
    STAB *stab;
    register char *s;
    int i;
	...
	if (!i && (dowarn || strEQ(mstr->str_ptr,"ALARM")))
	    warnx(1, "No such signal: SIG%s", mstr->str_ptr);
	if (strEQ(s,"IGNORE"))
#ifndef lint
    ...
    case 'L':
	{
		...
		warnx(0, "Can't break at that line\n");
	}
	break;
    case '#':
	stab = mstr->str_u.str_stab;
	...
	case '<':
		...
		fatalx(0, "setruid() not implemented");
#endif
#endif
	    uid = (int)getuid();
	    break;
	case '>':
		...
		fatalx(0, "seteuid() not implemented");
#endif
#endif
	    euid = (int)geteuid();
	    break;
	case '(':
		...
		fatalx(0, "setrgid() not implemented");
#endif
#endif
	    gid = (int)getgid();
	    break;
	case ')':
		...
		fatalx(0, "setegid() not implemented");
#endif
#endif
	    egid = (int)getegid();
	    break;
...
int sig;
{
    ...
    if (!sub) {
	if (dowarn)
	    warnx(2, "SIG%s handler \"%s\" not defined.\n",
...
STAB *
stabent(register char *name, int add)
{
    register STAB *stab;
    register STBP *stbp;
    int len;
...
void
stab_efullname(STR *str, STAB *stab)
{
...
register int max;
{
	...
	for (entry = defstash->tbl_array[i]; entry; entry = entry->hent_next) {
	    ...
	    warnx(1, "Possible typo: \"%s\"", stab_name(stab));
	}
    }
}
