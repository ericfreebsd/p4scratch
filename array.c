...
STR *
afetch(register ARRAY *ar, int key, int lval)
{
    STR *str;
...
bool
astore(register ARRAY *ar, int key, STR *val)
{
    int retval;
...
ARRAY *
afake(STAB *stab, register int size, register STR **strp)
{
    register ARRAY *ar;
...
bool
apush(register ARRAY *ar, STR *val)
{
    return astore(ar,++(ar->ary_fill),val);
}

STR *
apop(register ARRAY *ar)
{
    STR *retval;
...
void
aunshift(register ARRAY *ar, register int num)
{
    register int i;
...
int
alen(register ARRAY *ar)
{
    return ar->ary_fill;
}

void
afill(register ARRAY *ar, int fill)
{
    if (fill < 0)
	fill = -1;
    if (fill <= ar->ary_max)
