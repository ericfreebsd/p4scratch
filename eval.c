...
int
eval(register ARG *arg, int gimme, register int sp)
{
    register STR *str;
    register int anum;
    register int optype;
	...
	case A_READ:
	  ...
	  do_read:
	    ...
	    if (!fp && dowarn)
		warnx(1, "Read on closed filehandle <%s>",stab_ename(last_in_stab));
	...
	if (anum >= 1) {
		...
		warnx(0, "Right operand of x is not numeric");
	    str_sset(str,&str_no);
	}
	STABSET(str);
	break;
	...
	if ((value = str_gnum(st[2])) == 0.0)
	    fatalx(0, "Illegal division by zero");
#ifdef SLOPPYDIVIDE
	/* insure that 20./5. == 4. */
	{
	    double x;
	...
	if (dowarn) {
		...
		warnx(0, "Possible use of == on string value");
	}
	value = str_gnum(st[1]);
	value = (value == str_gnum(st[2])) ? 1.0 : 0.0;
	goto donumset;
...
/* use register in evaluating str_true() */
	...
	if (!form || !fp) {
	    if (dowarn) {
		if (form)
		    warnx(0, "No format for filehandle");
		else {
		    if (stab_io(stab)->ifp)
			warnx(0, "Filehandle only opened for input");
		    else
			warnx(0, "Write on closed filehandle");
		}
	    }
	    str_set(str, No);
	    STABSET(str);
	...
	goto donumset;
#else
	fatalx(0, "No dbm or ndbm on this machine");
#endif
    case O_DBMCLOSE:
#ifdef SOME_DBM
	anum = arg[1].arg_type & A_MASK;
	...
	if (!str || str == &str_undef)
	    fatalx(1, "Assignment to non-creatable value, subscript %d",anum);
	break;
    case O_LHELEM:
	tmpstab = arg[1].arg_ptr.arg_stab;
	tmps = str_get(st[2]);
	...
	if (!str || str == &str_undef)
	    fatalx(1, "Assignment to non-creatable value, subscript \"%s\"",tmps);
	...
	if (!stab_io(stab)) {
	    if (dowarn)
		warnx(0, "Filehandle never opened");
	    goto say_zero;
	}
	if (!(fp = stab_io(stab)->ofp)) {
	    if (dowarn)  {
		if (stab_io(stab)->ifp)
		    warnx(0, "Filehandle opened only for input");
		else
		    warnx(0, "Print on closed filehandle");
	    }
	    goto say_zero;
	}
	else {
	...
	if (!stio || !stio->ifp) {
		...
		if (optype == O_SYSWRITE)
		    warnx(0, "Syswrite on closed filehandle");
		else
		    warnx(0, "Send on closed socket");
	    }
	}
	else if (optype == O_SYSWRITE) {
	    if (maxarg > 4)
		warnx(0, "Too many args on syswrite");
	    if (maxarg == 4)
	...
	if (loop_ptr < 0) {
		...
		fatalx(0, "Can't return outside a subroutine");
	    fatalx(1, "Bad label: %s", maxarg > 0 ? tmps : "<null>");
	}
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function alarm");
	break;
#endif
    case O_SLEEP:
	if (maxarg < 1)
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function fork");
	break;
#endif
    case O_WAIT:
#ifdef HAS_WAIT
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function wait");
	break;
#endif
    case O_WAITPID:
#ifdef HAS_WAIT
...
donumset:
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function chown");
	break;
#endif
    case O_KILL:
#ifdef HAS_KILL
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function kill");
	break;
#endif
    case O_UNLINK:
    case O_CHMOD:
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function umask");
	break;
#endif
    ...
    case O_SHMREAD:
    case O_SHMWRITE:
	fatalx(0, "System V IPC is not implemented on this machine");
#endif /* not SYSVIPC */
    case O_RENAME:
	tmps = str_get(st[1]);
	tmps2 = str_get(st[2]);
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function link");
	break;
#endif
    case O_MKDIR:
	tmps = str_get(st[1]);
...
#define EACCES EPERM
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function getppid");
	break;
#endif
    case O_GETPGRP:
#ifdef HAS_GETPGRP
	...
	if (anum != 0)
	    fatalx(0, "POSIX getpgrp can't take an argument");
	value = (double)getpgrp();

	goto donumset;
#else
	fatalx(0, "The getpgrp() function is unimplemented on this machine");
	break;
#endif
    case O_SETPGRP:
#ifdef HAS_SETPGRP
	...
	goto donumset;
#else
	fatalx(0, "The setpgrp() function is unimplemented on this machine");
	break;
#endif
    case O_GETPRIORITY:
	...
	goto donumset;
#else
	fatalx(0, "The getpriority() function is unimplemented on this machine");
	break;
#endif
    case O_SETPRIORITY:
	...
	goto donumset;
#else
	fatalx(0, "The setpriority() function is unimplemented on this machine");
	break;
#endif
    case O_CHROOT:
#ifdef HAS_CHROOT
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function chroot");
	break;
#endif
    case O_FCNTL:
    case O_IOCTL:
	...
	goto donumset;
#else
	fatalx(0, "The flock() function is unimplemented on this machine");
	break;
#endif
    case O_UNSHIFT:
	ary = stab_array(arg[1].arg_ptr.arg_stab);
	...
	goto donumset;
#else
	fatalx(0, "Unsupported function symlink");
#endif
    case O_READLINK:
#ifdef HAS_SYMLINK
	if (maxarg < 1)
	...
	goto array_return;
    case O_SHOSTENT:
	sethostent((int)str_gnum(st[1]));
	value = (double)0;
	goto donumset;
    case O_SNETENT:
	setnetent((int)str_gnum(st[1]));
	value = (double)0;
	goto donumset;
    case O_SPROTOENT:
	setprotoent((int)str_gnum(st[1]));
	value = (double)0;
	goto donumset;
    case O_SSERVENT:
	setservent((int)str_gnum(st[1]));
	value = (double)0;
	goto donumset;
    case O_EHOSTENT:
	endhostent();
    ...
    case O_GETPEERNAME:
      badsock:
	fatalx(0, "Unsupported socket function");
#endif /* HAS_SOCKET */
    case O_SSELECT:
#ifdef HAS_SELECT
	...
	goto array_return;
#else
	fatalx(0, "select not implemented");
#endif
    case O_FILENO:
	if (maxarg < 1)
	    goto say_undef;
    ...
    case O_EPWENT:
    case O_SPWENT:
	fatalx(0, "Unsupported password function");
	break;
#endif
    case O_GGRNAM:
    case O_GGRGID:
    ...
    case O_EGRENT:
    case O_SGRENT:
	fatalx(0, "Unsupported group function");
	break;
#endif
    case O_GETLOGIN:
#ifdef HAS_GETLOGIN
	...
	str_set(str,tmps);
#else
	fatalx(0, "Unsupported function getlogin");
#endif
	break;
    case O_OPEN_DIR:
    case O_READDIR:
	...
	STABSET(str);
#else
	fatalx(0, "Unsupported function pipe");
#endif
	break;
    }
