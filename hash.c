...
STR *
hfetch(register HASH *tb, char *key, unsigned int klen, int lval)
{
    register char *s;
    register int i;
    register int hash;
...
bool
hstore(register HASH *tb, char *key, unsigned int klen, STR *val, register int hash)
{
    register char *s;
    register int i;
    register HENT *entry;
...
STR *
hdelete(register HASH *tb, char *key, unsigned int klen)
{
    register char *s;
    register int i;
    register int hash;
...
void
hclear(register HASH *tb, int dodbm)
{
    if (!tb)
	return;
...
void
hfree(register HASH *tb, int dodbm)
{
    if (!tb)
	return;
    hfreeentries(tb,dodbm);
...
char *
hiterkey(register HENT *entry, int *retlen)
{
...
STR *
hiterval(register HASH *tb, register HENT *entry)
{
#ifdef SOME_DBM
    datum key, content;
...
#else
    if (dbmrefcnt++)
	fatalx(0, "Old dbm can only open one database");
...
register HASH *tb;
{
	...
	warnx(0, "Close on unopened dbm file");
}
...
register STR *str;
{
    ...
    if (error) {
	if (errno == EPERM)
	    fatalx(0, "No write permission to dbm file");
	warnx(3, "dbm store returned %d, errno %d, key \"%s\"",error,errno,key);
#ifdef HAS_NDBM
