char rcsid[] = "$RCSfile: perl.c from p4l perl 4.1\n";
/*
...
#ifdef IAMSUID
#undef IAMSUID
    fatalx(0, "suidperl is no longer needed since the kernel can now execute\n\
setuid perl scripts securely.\n");
    ...
    gid = (int)getgid();
    egid = (int)getegid();
    sprintf(patchlevel,"%d.%d.%d\0", MAJORVERSION, MINORVERSION, PATCHLEVEL);
#ifdef MSDOS
    /*
	...
	case 'e':
		...
		fatalx(0, "No -e allowed in setuid scripts");
#endif
	    if (!e_fp) {
		int fd;
		...
		if (fd == -1)
		    fatalx(0, "Can't mkstemp()");
		e_fp = fdopen(fd,"w");
		if (!e_fp) {
		    close(fd);
		    fatalx(0, "Cannot open temporary file");
		}
	    }
	    if (argv[1]) {
		fputs(argv[1],e_fp);
...
#ifdef TAINT
	    if (euid != uid || egid != gid)
		fatalx(0, "No -I allowed in setuid scripts");
#endif
	    str_cat(str,"-");
	    str_cat(str,s);
	    str_cat(str," ");
...
#ifdef TAINT
	    if (euid != uid || egid != gid)
		fatalx(0, "No -P allowed in setuid scripts");
#endif
	    preprocess = TRUE;
	    s++;
	    goto reswitch;
...
#ifdef TAINT
	    if (euid != uid || egid != gid)
		fatalx(0, "No -s allowed in setuid scripts");
#endif
	    doswitches = TRUE;
	    s++;
	    goto reswitch;
	...
	case 0:
	    break;
	default:
	    fatalx(1, "Unrecognized switch: -%s",s);
	}
    }
  switch_end:
    scriptname = argv[0];
	...
	if (fflush(e_fp) || ferror(e_fp) || fclose(e_fp))
	    fatalx(1, "Can't write to temp file for -e: %s", strerror(errno));
	argc++,argv--;
...
#define PRIVLIB "/usr/share/perl"
	...
	if (!xfound)
	    fatalx(1, "Can't execute %s", xfailed ? xfailed : scriptname );
	if (xfailed)
	    Safefree(xfailed);
 ...
 %s | %s -C %s %s",
...
#ifdef TAINT
	if (euid != uid || egid != gid)
	    fatalx(0, "Can't take set-id script from stdin");
#endif
	rsfp = stdin;
    }
    else
	...
	if (euid && stat(stab_val(curcmd->c_filestab)->str_ptr,&statbuf) >= 0 &&
	  statbuf.st_mode & (S_ISUID|S_ISGID)) {
	    ...
	    fatalx(0, "Can't do setuid\n");
	}
#endif
#endif
	fatalx(2, "Can't open perl script \"%s\": %s\n",
	...
	fatalx(1, "Can't stat script \"%s\"",origfilename);
#ifdef IAMSUID
    {
	struct statfs stfs;

	if (fstatfs(fileno(rsfp),&stfs) < 0)
	    fatalx(1, "Can't statfs filesystem of script \"%s\"",origfilename);

	if (stfs.f_flags & MNT_NOSUID)
	    fatalx(0, "Permission denied");
    }
#endif /* IAMSUID */
	 ...
	 */
	{
		...
		fatalx(0, "Can't swap uid and euid");	/* really paranoid */
	    if (stat(stab_val(curcmd->c_filestab)->str_ptr,&tmpstatbuf) < 0)
		fatalx(0, "Permission denied");	/* testing full pathname here */
...
(Filename of set-id script was %s, uid %d gid %d.)\n\nSincerely,\nperl\n",
		...
		fatalx(0, "Can't reswap uid and euid");
	...
	if (statbuf.st_mode & S_IWOTH)
	    fatalx(0, "Setuid/gid script is writable by world");
	...
	if (strEQ(validarg," PHOOEY ") ||
	    ...
	    fatalx(0, "Args must match #! line");

#ifndef IAMSUID
	if (euid != uid && (statbuf.st_mode & S_ISUID) &&
	    euid == statbuf.st_uid)
	    if (!do_undump)
		fatalx(0, "YOU HAVEN'T DISABLED SET-ID SCRIPTS IN THE KERNEL YET!\n\
FIX YOUR KERNEL, PUT A C WRAPPER AROUND THIS SCRIPT, OR USE -u AND UNDUMP!\n");
	...
	fatalx(0, "-P not allowed for setuid/setgid script\n");
    else
	fatalx(0, "Script is not setuid/setgid in suidperl\n");
#else
	...
	execv(buf, origargv);	/* try again */
	fatalx(0, "Can't run setuid script with taint checks");
    }
#endif /* TAINT */
#endif /* IAMSUID */
#else /* !DOSUID */
...
FIX YOUR KERNEL, PUT A C WRAPPER AROUND THIS SCRIPT, OR USE -u AND UNDUMP!\n");
	...
	if ((s = str_gets(linestr, rsfp, 0)) == Nullch)
	    fatalx(0, "No Perl script found in input\n");
	if (*s == '#' && s[1] == '!' && instr(s,"perl")) {
		...
		fatalx(1, "Can't chdir to %s",cddir);
	}
    }
	...
	if (minus_c)
	    fatalx(1, "%s had compilation errors.\n", origfilename);
	else {
	    fatalx(1, "Execution of %s aborted due to compilation errors.\n",
		origfilename);
	}
    }
  ...
  just_doit:		/* come here if running an undumped a.out */
    ...
    if (goto_targ)
	fatalx(1, "Can't find label \"%s\"--aborting",goto_targ);
    exit(0);
    /* NOTREACHED */
}
...
int
do_eval(STR *str, int optype, HASH *stash, int savecmd, int gimme, int *arglast)
{
	...
	if (retval) {
	    if (optype == O_REQUIRE)
		fatalx(1, "%s", str_get(stab_val(stabent("@",TRUE))));
	}
	else {
	    curcmd = oldcurcmd;
		...
		fatalx(1, "%s did not return a true value", specfilename);
	}
    }
    curcmd = oldcurcmd;
    return sp;
...
int
do_try(CMD *cmd, int gimme, int *arglast)
{
    STR **st = stack->ary_array;
...
char *s;
{
...
#ifdef TAINT
	if (euid != uid || egid != gid)
	    fatalx(0, "No -d allowed in setuid scripts");
#endif
	perldb = TRUE;
	s++;
	return s;
...
#else
	warnx(0, "Recompile perl with -DDEBUGGING to use -D switch\n");
#endif
	/*SUPPRESS 530*/
	for (s++; isDIGIT(*s); s++) ;
	return s;
	...
	*s = '\0';
	...
	if (*++s) {
	    ...
	    fatalx(0, "No space allowed after -I");
	break;
    case 'l':
	minus_l = TRUE;
	s++;
	...
	return s;
    case 'v':
	fputs("\nThis is perl4 aka p4l, version ",stdout);
	fputs(patchlevel,stdout);
	fputs("\n\n",stdout);
	fputs(rcsid,stdout);
	fputs("\nCopyright (c) 1989-2026, Larry Wall and others\n",stdout);
#ifdef MSDOS
...
GNU General Public License, which may be found in the Perl 4.0 source kit.\n",stdout);
	...
	break;
    default:
	fatalx(1, "Switch meaningless after -x: -%s",s);
    }
    return Nullch;
}
