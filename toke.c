...
check_uni() {
    ...
    ch = *s;
    *s = '\0';
    warnx(1, "Warning: Use of \"%s\" without parens is ambiguous", last_uni);
    *s = ch;
}
  ...
  retry:
	...
	if ((*s & 127) == '(') {
	    ...
	    *s++ = '}';
	    ...
	    warnx(1, "Unrecognized character \\%03o ignored", *s++ & 255);
	goto retry;
    }
#endif
    switch (*s) {
...
#define SNARFWORD \
	...
	*d = '\0'; \
	...
	if (!isALPHA(*s) && *s != '_' && *s != '\'') {
	    warnx(0, "Spurious backslash ignored");
	    goto retry;
	}
	/* FALL THROUGH */
    case '_':
	...
	if (strEQ(d,"for") || strEQ(d,"foreach")) {
		...
		s++;
	    if (isALPHA(*s))
		fatalx(0, "Missing $ on loop variable");
	    OPERATOR(FOR);
	}
	if (strEQ(d,"format")) {
	    d = bufend;
	...
	case 'o':
		...
		if (*s == ';' || *s == ')')		/* probably a close */
		    fatalx(0, "sort is now a reserved word");
		if (isALPHA(*s) || *s == '_') {
		    /*SUPPRESS 530*/
		    for (d = s; isALNUM(*d); d++) ;
...
checkcomma(register char *s, char *name, char *what)
{
	...
	if (!w || !*w || !index(";|}", *w))	/* an advisory hack only... */
	    warnx(1, "%s (...) interpreted as function",name);
    }
    while (s < bufend && isSPACE(*s))
	s++;
    if (*s == '(')
	...
	if (*s == ',') {
	    ...
	    *s = ',';
	    if (w)
		return;
	    fatalx(1, "No comma allowed after %s", what);
	}
    }
}

char *
scanident(register char *s, register char *send, char *dest, STRLEN destlen)
{
    register char *d;
    register char *e;
    int brackets = 0;
...
void
scanconst(SPAT *spat, char *string, int len)
{
    register STR *tmpstr;
    register char *t;
    register char *d;
...
char *
scanstr(char *start, int in_what)
{
    register char *s = start;
    register char term;
    register char *d;
	...
	*d = '\0';
	...
	if (s < bufend)
	    ...
	    fatalx(0, "Unterminated <> operator");
