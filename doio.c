...
bool
do_open(STAB *stab, register char *name, int len)
{
    FILE *fp;
...
nextargv(register STAB *stab)
{
	...
	if (do_open(stab,oldname,stab_val(stab)->str_cur)) {
		...
		if (!S_ISREG(filemode)) {
		    warnx(1, "Can't do inplace edit: %s is not a regular file",
		      oldname );
		    do_close(stab,FALSE);
		    str_free(str);
		    continue;
		...
		if (*inplace) {
			...
			warnx(1, "Can't do inplace edit: %s > 14 characters",
			  str->str_ptr );
			do_close(stab,FALSE);
			str_free(str);
			continue;
			...
			warnx(3, "Can't rename %s to %s: %s, skipping file",
...
#else
		    fatalx(0, "Can't do inplace edit without backup");
#endif
		}
		...
		if (!do_open(argvoutstab,str->str_ptr,str->str_cur)) {
		    warnx(2, "Can't do inplace edit on %s: %s",
...
#ifdef HAS_PIPE
void
do_pipe(STR *str, STAB *rstab, STAB *wstab)
{
    register STIO *rstio;
    register STIO *wstio;
    int fd[2];
...
bool
do_close(STAB *stab, bool explicit)
{
    bool retval = FALSE;
    register STIO *stio;
    int status;
	...
	if (dowarn && explicit)
	    warnx(1, "Close on unopened file <%s>",stab_ename(stab));
	return FALSE;
    }
    if (stio->ifp) {
	if (stio->type == '|') {
...
nuts:
    if (dowarn)
	warnx(0, "seek() on unopened file");
    errno = EBADF;
    return FALSE;
}

int
do_ctl(int optype, STAB *stab, int func, STR *argstr)
{
    register STIO *stio;
    register char *s;
    int retval;
...
#define IOCPARM_LEN(x)  (((x) >> 16) & IOCPARM_MASK)
...
#ifdef DOSISH
	fatalx(0, "fcntl is not implemented");
#else
#ifdef HAS_FCNTL
	...
	if (s[argstr->str_cur] != 17)
	    fatalx(0, "Return value overflowed string");
...
int
do_stat(STR *str, register ARG *arg, int gimme, int *arglast)
{
...
int					/*SUPPRESS 590*/
do_truncate(STR *str, register ARG *arg, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
    register int sp = arglast[0] + 1;
    ...
    return sp;
#else
    fatalx(0, "truncate not implemented");
#endif
}

int
looks_like_number(STR *str)
{
    register char *s;
    register char *send;
...
bool
do_print(register STR *str, FILE *fp)
{
    register char *tmps;

    if (!fp) {
	if (dowarn)
	    warnx(0, "print to unopened file");
	errno = EBADF;
	return FALSE;
    }
    if (!str)
...
bool
do_aprint(register ARG *arg, register FILE *fp, int *arglast)
{
...
mystat(ARG *arg, STR *str)
{
	...
	if (stio && stio->ifp) {
		...
		return laststatval;
	    if (dowarn)
		warnx(1, "Stat on unopened file <%s>",
...
int
mylstat(ARG *arg, STR *str)
{
    if (arg[1].arg_type & A_DONT) {
	if (arg[1].arg_ptr.arg_stab == defstab) {
	    if (laststype != O_LSTAT)
		fatalx(0, "The stat preceding -l _ wasn't an lstat");
	    return laststatval;
	}
	fatalx(0, "You can't use -l on a filehandle");
    }
...
STR *str;
{
	...
	if (stio && stio->ifp) {
	    ...
	    if (dowarn)
		warnx(1, "Test on unopened file <%s>",
		  stab_ename(arg[1].arg_ptr.arg_stab));
	    errno = EBADF;
	    return &str_undef;
	}
...
#ifdef HAS_SOCKET
int
do_socket(STAB *stab, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register STIO *stio;
...
int
do_connect(STAB *stab, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register STIO *stio;
...
int
do_listen(STAB *stab, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register STIO *stio;
...
int
do_shutdown(STAB *stab, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register STIO *stio;
...
int
do_sopt(int optype, STAB *stab, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register STIO *stio;
...
nuts:
    if (dowarn)
	warnx(0, "[gs]etsockopt() on closed fd");
    st[sp] = &str_undef;
    errno = EBADF;
    return sp;
...
int
do_getsockname(int optype, STAB *stab, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register STIO *stio;
...
nuts:
    if (dowarn)
	warnx(0, "get{sock,peer}name() on closed fd");
    errno = EBADF;
nuts2:
    st[sp] = &str_undef;
    return sp;
...
int
do_ghent(int which, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
	...
	hent = gethostent();
#else
	fatalx(0, "gethostent not implemented");
#endif
...
int
do_gnent(int which, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
    register int sp = arglast[0];
    register char **elem;
...
int
do_gpent(int which, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
    register int sp = arglast[0];
    register char **elem;
...
int
do_gsent(int which, int gimme, int *arglast)
{
    register ARRAY *ary = stack;
    register int sp = arglast[0];
    register char **elem;
...
#ifdef HAS_SELECT
int
do_select(int gimme, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[0];
    register int i;
...
#ifdef HAS_SOCKET
int
do_spair(STAB *stab1, STAB *stab2, int *arglast)
{
    register STR **st = stack->ary_array;
	...
	return FALSE;
#else
    fatalx(0, "Socketpair unimplemented");
#endif
...
int
do_gpwent(int which, int gimme, int *arglast)
{
#ifdef I_PWD
    register ARRAY *ary = stack;
    register int sp = arglast[0];
    ...
    return sp;
#else
    fatalx(0, "password routines not implemented");
#endif
}

int
do_ggrent(int which, int gimme, int *arglast)
{
#ifdef I_GRP
    register ARRAY *ary = stack;
    register int sp = arglast[0];
    ...
    return sp;
#else
    fatalx(0, "group routines not implemented");
#endif
}

int
do_dirop(int optype, STAB *stab, int gimme, int *arglast)
{
...
#endif
phooey:
    fatalx(0, "Unimplemented directory operation");
}

int
apply(int type, int *arglast)
{
    register STR **st = stack->ary_array;
    register int sp = arglast[1];
    register int items = arglast[2] - sp;
	...
	if (--items > 0) {
		...
		if (*s == 'S' && s[1] == 'I' && s[2] == 'G')
		    s += 3;
		if (!(val = whichsig(s)))
		    fatalx(1, "Unrecognized signal name \"%s\"",s);
	    }
	    else
		val = (int)str_gnum(st[sp]);
	    if (val < 0) {
...
int
cando(int bit, int effective, register struct stat *statbufp)
{
#ifdef DOSISH
...
int
ingroup(int testgid, int effective)
{
...
#if !defined(HAS_MSG) || !defined(HAS_SEM) || !defined(HAS_SHM)
    default:
	fatalx(1, "%s not implemented", opname[optype]);
#endif
    }
...
#else
    fatalx(0, "msgsnd not implemented");
#endif
}
...
int *arglast;
{
    ...
    return ret;
#else
    fatalx(0, "msgrcv not implemented");
#endif
}
...
#else
    fatalx(0, "semop not implemented");
#endif
}
...
int *arglast;
{
    ...
    return shmdt(shm);
#else
    fatalx(0, "shm I/O not implemented");
#endif
}
