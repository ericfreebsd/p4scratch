...
int
cmd_exec(CMD *VOLATILE cmdparm, VOLATILE int gimme, VOLATILE int sp)
{
...
tail_recursion_entry:
	...
	if (cmd->c_label && strEQ(go_to,cmd->c_label))
		...
		if (match) {
		    ...
		    switch (match) {
		    default:
			fatalx(1, "longjmp returned bad value (%d)",match);
...
until_loop:
	...
	if (match) {
	    ...
	    switch (match) {
	    default:
		fatalx(0, "longjmp returned bad value (%d)",match);
	    case O_LAST:
		if (lastretstr) {
		    retstr = lastretstr;
		    newsp = -2;
...
int
copyopt(register CMD *cmd, register CMD *which)
{
...
void
savelist(register STR **sarg, int maxsarg)
{
    register STR *str;
    register int i;
...
restorelist(int base)
{
    ...
    if (base < -1)
	fatalx(0, "panic: corrupt saved stack index");
	...
	case SS_SCSV:				/* callsave structure */
	    ...
	    break;
	default:
	    fatalx(0, "panic: restorelist inconsistency");
	}
    }
}
